...
class _TransactionScreenState extends ConsumerState<TransactionScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final subtotal =
        ref.read(cartControllerProvider.notifier).totalAmount.toDouble();
    ...
    final paymentConfig = workspaceId == null
        ? null
        : ref.watch(workspacePaymentConfigProvider(workspaceId));
                          ...
                          child: paymentConfig == null
                              ...
                              : paymentConfig.when(
                                  ...
                                  error: (_, __) => const Text(
                                    'Gagal memuat konfigurasi pembayaran lapak.',
                                    textAlign: TextAlign.center,
                                  ),
                                  data: (config) =>
                                      _buildPaymentMethodDetails(theme, config),
                                ),
                        ),
  ...
  Widget _buildPaymentMethodDetails(
      ThemeData theme, PaymentConfigModel config) {
    if (_selectedPaymentMethod == PaymentMethod.qris) {
      if (config.qrisImageUrl == null || config.qrisImageUrl!.isEmpty) {
        return const Text('QRIS lapak belum dikonfigurasi.',
            textAlign: TextAlign.center);
      }
          ...
          children: [
            ...
            Image.network(
              ...
              errorBuilder: (_, __, ___) =>
                  const Icon(Icons.broken_image_rounded, size: 100),
            ),
            const SizedBox(height: 16),
            Text('QRIS ${ref.read(currentWorkspaceProvider)?.name ?? ''}',
      ...
      if (config.transferAccounts.isEmpty) {
        return const Text('Rekening transfer lapak belum dikonfigurasi.',
            textAlign: TextAlign.center);
      }
      ...
      return Container(
        ...
        child: Column(
          spacing: 12,
          children: [
            ...config.transferAccounts.map((account) => _buildBankTransferCard(
                  theme,
                  account.bankName,
  ...
  Widget _buildBankTransferCard(ThemeData theme, String bankName,
