...
BEGIN
  ...
  WHERE name = 'workspace_api_key_encryption'
  LIMIT 1;

  IF v_secret IS NULL OR v_secret = '' THEN
    RAISE EXCEPTION
      'Create the Supabase Vault secret workspace_api_key_encryption before applying this migration';
  END IF;
    ...
    LOOP
      ...
      BEGIN
        v_plaintext := extensions.pgp_sym_decrypt(
          decode(v_ciphertext, 'hex'),
          'SERVER_SECRET'
        );
      EXCEPTION WHEN OTHERS THEN
        RAISE EXCEPTION
          'Could not re-encrypt an existing API key for workspace %',
          v_workspace.id;
      END;

      v_keys := v_keys || jsonb_build_array(jsonb_build_object(
        'provider', v_item->>'provider',
        'key', encode(extensions.pgp_sym_encrypt(v_plaintext, v_secret), 'hex')
      ));
    END LOOP;
...
CREATE OR REPLACE FUNCTION public.workspace_api_key_encryption_secret()
RETURNS TEXT
LANGUAGE sql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.workspace_api_key_encryption_secret() FROM PUBLIC, anon, authenticated;
...
DECLARE
  ...
  v_provider TEXT;
  v_ciphertext TEXT;
  v_secret TEXT := public.workspace_api_key_encryption_secret();
BEGIN
  ...
  IF v_secret IS NULL OR v_secret = '' THEN
    RAISE EXCEPTION 'API key encryption secret is not configured';
  END IF;
  ...
  LOOP
    ...
    ELSE
      v_encrypted_key := encode(
        extensions.pgp_sym_encrypt(v_key, v_secret),
        'hex'
      );
    END IF;
